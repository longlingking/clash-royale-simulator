...
class Dreamer(nn.Module):
    ...
    def update(self, replay_buffer):
        ...
        if self.rep_loss == "dreamerpro":
            self.ema_update()
        metrics = {}
        # CPU autocast officially supports bfloat16 only; fp16 is CUDA-only.
        _ac_dtype = torch.float16 if self.device.type == "cuda" else torch.bfloat16
        with autocast(device_type=self.device.type, dtype=_ac_dtype):
